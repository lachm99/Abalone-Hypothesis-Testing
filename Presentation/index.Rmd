...
```{r}
sjPlot::tab_model(step.back.aic, step.back.aic, show.ci = FALSE,show.aic = TRUE,dv.labels = c("Forward model","Backward model"))
```
...
.brand-red[# Assumption Checking]

.pull-right[## .brand-red[Residual vs Fitted].
```{r, warning = FALSE}
# autoplot(step.fwd.aic,which=1)
x = transformed
x = x %>% mutate(lfitted = step.back.aic$fitted.values, lresid = step.back.aic$residuals )
ggplot(x, aes(x = lfitted ,y = lresid)) + geom_point(size = 3) +geom_smooth()
```
]
...
```{r}
M0 = lm(sqrt_log_rings ~ ., transformed)
# drop1(M1, test = "F")

step.back = step(M0, direction = "backward", trace = FALSE)
# summary(step.back.aic)

sjPlot::tab_model(step.back, show.ci = FALSE,show.aic = TRUE,dv.labels = c("Final model"))
```
...
$$
\widehat{\operatorname{sqrt\_log\_rings}} = 1.45 + 0.2(\operatorname{log\_whole}) - 0.19(\operatorname{log\_shucked}) - 0.02(\operatorname{log\_viscera}) + 0.11(\operatorname{log\_shell}) + 0.07(\operatorname{log\_diam}) - 0.08(\operatorname{log\_length}) + 0.12(\operatorname{sqrt\_height}) + \epsilon
$$
