...
# Abstract
abstract: |
  We investigated several predictive models for predicting the age of the abalone. In order to estimate the age by relevant variables which are the physical characteristics of the abalone, we conducted a simple linear regression, multiple regression, backward and forward selection using Akaike Information Criterion (AIC). Additionally, General Linear Model (GLM) was also used to find out the best fitted model.
