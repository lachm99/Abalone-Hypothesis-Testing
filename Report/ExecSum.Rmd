...
Provided the linearity assumptions with respect to the dependant variable are satisfied, a model search was justfiied by the Akaike information criterion through a backward and forward variable seleciton. After conducting the relevant seacrch it was found that the forward model did not include log of diameter and log of length while the backward approach included all variables, shown in the table below. 
...
forward_plot1 = ggplot(x, aes(x = lfitted ,y = lresid)) + geom_jitter(size=0.2, alpha=0.5, col="black") + geom_smooth(size=0.5) +
...
forward_plot2 = ggplot(step.fwd.aic, aes(sample = step.fwd.aic$residuals)) + stat_qq(alpha=0.5, shape=1) + stat_qq_line(colour="blue") + theme(text=element_text(size=6), panel.background = element_blank(), panel.grid.major = element_line(colour="white"), panel.grid.minor = element_line(colour="white"), panel.border = element_rect(colour="steelblue", fill=NA, size=1))
...
back_plot1 = ggplot(x, aes(x = lfitted ,y = lresid)) + geom_jitter(size=0.2, alpha=0.5, col="black") + geom_smooth(size=0.5) +
  labs(x = "Fitted", y = "Residuals") + theme(text=element_text(size=6), panel.background = element_blank(), panel.grid.major = element_line(colour="white"), panel.grid.minor = element_line(colour="white"), panel.border = element_rect(colour="steelblue", fill=NA, size=1))

# autoplot(step.fwd.aic,which=2)
back_plot2 = ggplot(step.back.aic, aes(sample = step.back.aic$residuals)) + stat_qq(alpha=0.5, shape=1) + stat_qq_line(colour="blue") + theme(text=element_text(size=6), panel.background = element_blank(), panel.grid.major = element_line(colour="white"), panel.grid.minor = element_line(colour="white"), panel.border = element_rect(colour="steelblue", fill=NA, size=1))
...
```{r, fig.width=3, fig.height=1.25, cache=TRUE}
