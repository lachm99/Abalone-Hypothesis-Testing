...
```{r, echo=FALSE}
ggplot(unfiltered) + aes(height) + geom_boxplot(color = "#00AFBB")
```
...
Conducting this analysis was fruitful. Our results demonstrate that we can indeed construct a model that will approximate an Abalone's age without needing any arduous ring-counting. Such a tool is very useful for monitoring large marine ecosystems, where there are potentially thousands of observations needing classification and not enough time to exactly calculate the age of each Abalone.  
However, this also exposes one limitation of the data. Our data only pertains to _Haliotis_ _Rubra_, and so we cannot claim that our model accounts for species, or will even perform generally among Haliotes.
