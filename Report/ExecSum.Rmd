...
tables = sjPlot::tab_model(step.fwd.aic, step.back.aic, show.ci = FALSE,show.aic = TRUE,dv.labels = c("Forward model","Backward model"), CSS = list(css.tdata = "padding:2px;text-align:left;vertical-align:top;"))
# cat(tab$page.style)
```
...
```{r, warning = FALSE, echo = FALSE, fig.width=3, fig.height=1.25, cache=TRUE}
...
ggplot(x, aes(x = lfitted ,y = lresid)) + geom_point(size = 1) + geom_smooth() +
  labs(title="Residuals vs Fitted", x = "Fitted", y = "Residuals") + theme(text=element_text(size=8))
```

```{r, warning = FALSE, echo = FALSE, fig.width=3, fig.height=2, cache=TRUE}
...
```{r, warning = FALSE, echo = FALSE, fig.width=3, fig.height=1.25, cache=TRUE, fig.show="hold", out.width="50%"}
# autoplot(step.fwd.aic,which=1)
x = transformed
...
results = resamples(list(forward = cv_forward,
                         back = cv_back))
ggplot(results, metric = "RMSE") + labs(y = "RMSE") + theme(text=element_text(size=8))

```

```{r, fig.width=3, fig.height=1.25, cache=TRUE}
ggplot(results, metric = "MAE") + labs(y = "MAE")+ theme(text=element_text(size=8))
